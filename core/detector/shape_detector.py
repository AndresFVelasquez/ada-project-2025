...
class ShapeDetector:
    ...
    def detect_shapes(point_list: list) -> list:
        ...
        for combo in tri_combinations:
            if ShapeDetector.is_right_triangle(combo):
                fig = FigureFactory.create_figure(combo, FigureType.RIGHT_TRIANGLE)
                figures.append(fig)
            elif ShapeDetector.is_acute_triangle(combo):
                fig = FigureFactory.create_figure(combo, FigureType.ACUTE_TRIANGLE)
                figures.append(fig)
        ...
        for combo in quad_combinations:
            if ShapeDetector.is_square(combo):
                fig = FigureFactory.create_figure(combo, FigureType.SQUARE)
                figures.append(fig)
            elif ShapeDetector.is_rectangle(combo):
                fig = FigureFactory.create_figure(combo, FigureType.RECTANGLE)
                figures.append(fig)
    ...
    def compute_sides(vertices: list) -> list[float]:
        ...
        n = len(vertices)
        if n not in (3, 4):
            raise ValueError("compute_sides solo acepta triángulos o cuadriláteros.")
        
        sides = []
        for i in range(n):
            p1 = vertices[i]
            p2 = vertices[(i + 1) % n] # conecta con el siguiente, y el último con el primero
    ...
    def compute_vectors(vertices: list) -> list:
        ...
        n = len(vertices)
        if n not in (3, 4):
            raise ValueError("compute_vectors solo acepta triángulos o cuadriláteros.")
    ...
    def is_rectangle(points: list) -> bool:
        ...
        # 1. Verificar ángulos rectos
        for i in range(4):
            v1 = vectors[i]
            v2 = vectors[(i + 1) % 4]

            if not ShapeDetector.is_perpendicular(v1, v2):
                return False
